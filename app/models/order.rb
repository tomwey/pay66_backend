class Order < ActiveRecord::Base
  ...
  def send_alipay(auth_code)
    ...
    code,res = Alipay::Pay.pay2merch(app_config.app_id, ma.app_auth_token, self.order_no, auth_code, self.title || "付款#{(self.money - (self.discount_money || 0))/100.0}元", ma.userid, self._money, self._discount_money || 0.0, "#{self.operator.id}", "#{self.shop.id}", "#{self.device.serial_no}",app_config.sys_pid, app_config.private_key, app_config.pub_key)
    if code == 0
